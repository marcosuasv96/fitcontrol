...
import { supabase } from '@/lib/supabase'; // Usando tu conector unificado central

// Definición de tipos formales adaptados a tu base de datos de producción
...
export default function DashboardAdmin() {
  ...
  useEffect(() => {
    async function cargarMétricasYListas() {
      ...
      // 3. Obtener licencias vigentes mapeando tus columnas reales de producción (nombre, precio, tipo_plan)
          ...
          if (lic.planes && lic.plan_id) {
            ...
            switch (vigencia) {
              ...
              case 'anual':
                ...
                break;
              case 'permanente':
                sumaMensual += precio * 0.05; // 5% de amortización mensual contable estándar para vitalicios
                break;
              default:
                sumaMensual += precio;
            }
  ...
  return (
    <div className="space-y-8">
      {/* Encabezado */}
      <div>
        <h2 className="text-2xl font-black text-[#232529] tracking-tight">Resumen Ejecutivo Global</h2>
        <p className="text-slate-400 text-xs mt-1 font-medium">Métricas estructurales y rendimiento de licencias de FitControl en tiempo real.</p>
      </div>

      {/* Grid de Tarjetas de Indicadores */}
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* Card 1: Clientes */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between min-h-[140px]">
          <div>
            ...
            <p className="text-3xl font-black text-[#232529] mt-3 font-mono">{loading ? '...' : totalClientes}</p>
          </div>
          <span className="text-[10px] text-slate-400 font-medium block">Establecimientos registrados</span>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between min-h-[140px]">
          <div>
            ...
            <p className="text-3xl font-black text-emerald-600 mt-3 font-mono">{loading ? '...' : licenciasActivas}</p>
          </div>
          <span className="text-[10px] text-emerald-600 font-bold block">● Servidores en production</span>
        </div>

        {/* Card 3: Alertas Vencimiento */}
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between min-h-[140px]">
          <div>
            <div className="flex justify-between items-center text-slate-400">
            ...
            <p className="text-3xl font-black text-amber-500 mt-3 font-mono">{loading ? '...' : licenciasPorVencerCount}</p>
          </div>
          <span className="text-[10px] text-slate-400 font-medium block">Vencimientos en los prox. 7 días</span>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col justify-between min-h-[140px]">
          <div>
            ...
            {/* Cambiado el formateador regional a pesos es-MX */}
            <p className="text-3xl font-black text-[#232529] mt-3 font-mono">${loading ? '...' : ingresoEstimado.toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-xs text-slate-400 font-bold font-sans">MXN</span></p>
          </div>
          <span className="text-[10px] text-slate-400 font-medium block">Flujo de caja SaaS amortizado</span>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden flex flex-col justify-between">
          <div>
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase font-mono">Top 5 Inminentes</span>
            </div>
                      ...
                      <tr key={lic.id} className="hover:bg-slate-50/50 transition">
                        ...
                        <td className="p-3.5">
                          {lic.dias_restantes < 0 ? (
                            ...
                            <span className="bg-amber-100 text-amber-800 font-black px-2.5 py-1 rounded-md text-[10px] animate-pulse">
                              Vence Hoy
                            </span>
                          ) : lic.dias_restantes <= 7 ? (
                            <span className="bg-amber-50 border border-amber-100 text-amber-700 font-bold px-2.5 py-1 rounded-md text-[10px]">
                              Expira en {lic.dias_restantes} días
                            </span>
                          ) : (
                            <span className="bg-slate-100 text-slate-600 font-medium px-2.5 py-1 rounded-md text-[10px]">
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden flex flex-col justify-between">
          <div>
            <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
              <h3 className="text-xs font-bold text-[#232529] uppercase tracking-wider font-sans flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-[#F47521] uppercase font-mono tracking-wider">Métricas de Éxito</span>
            </div>
      ...
      {/* Registro Informativo Inferior */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-start gap-4 shadow-md">
        <div className="p-2 bg-slate-800 rounded-lg border border-slate-700 text-[#F47521]">
        ...
        <div>
          <h4 className="font-bold text-slate-200 text-sm tracking-wide">Algoritmo Analítico Automatizado</h4>
